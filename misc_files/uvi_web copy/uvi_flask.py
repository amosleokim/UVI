...
from methods import top_parent_id, find_matching_ids, find_matching_elements, unique_id, mongo_to_json, formatted_def, get_themrole_fields, get_themrole_fields_undefined, full_class_hierarchy_tree, get_pred_fields, get_constant_fields, get_verb_specific_fields, remove_object_ids, colored_pb_example, vn_sanitized_class, get_themrole_fields_undefined
...
@app.route('/_filtered_results', methods=['GET','POST'])
def filtered_results():
	if request.form.get('resource_key') == 'VerbNet':
		#'vn_class_ids' contains all class id's that were checked in <form id='vn_filtered_elements'>
		vn_class_ids = request.form.getlist('vn_class_checkbox')
		sort_behavior = request.form['sort_behavior']
		#'matched_elements' contains full element representations (in Mongo's JSON-like notation) for all checked classes
		if sort_behavior == 'alpha':
			matched_elements = mongo.db.verbnet.find({'class_id': {'$in': vn_class_ids}}).sort('class_id', 1)
		elif sort_behavior == 'num':
			matched_elements = mongo.db.verbnet.find({'class_id': {'$in': vn_class_ids}}).sort('num_comparison_id', 1)
		...
		fn_names = request.form.getlist('fn_class_checkbox')
		matched_elements = mongo.db.framenet.find({'name': {'$in': fn_names}}).sort('name', 1)
		return render_template('render_framenet.html', fn_elements = matched_elements)

	elif request.form.get('resource_key') == 'PropBank':
		pb_lemmas = request.form.getlist('pb_class_checkbox')
		matched_elements = mongo.db.propbank.find({'predicates.lemma': {'$in': pb_lemmas}}).sort('frameset_id', 1)
		return render_template('render_propbank.html', pb_elements = matched_elements)
